...
model_dirs =  ['/home/fa19/Documents/Benchmarking/results/monet/scan_age/qb9910/best_model']
...
def numpy_to_occlusion(ARR, vertices_list = vertices_list):
    ...
    for m in range(40962):
        sol[m,:] = np.mean( np.abs(ARR[vertices_list[m]]), 0)
...
for model_dir in model_dirs:
    ...
    for filename in os.listdir(raw_path):
        if 'occlusion' in filename and '.npy' in filename and 'scan_age' in filename:
            raw_filename = filename.split('.')[0]
            arr = np.load(raw_path + '/' + filename)
            if len(arr.shape)>1:
                if arr.shape[1] == 5:
                    arr = arr - 0.7989616394042969
