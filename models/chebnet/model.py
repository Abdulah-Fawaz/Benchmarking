...
class GraphResidualBlock(nn.Module):
    ...
    def __init__(self, inchannels, outchannels, conv_style, activation_function, edges_level, downsample=None, device = 'cuda'):
        ...
        self.edges = edges_list[edges_level].to('cuda')
    def forward(self, x):
        residual = x
        ...
        if self.downsample is not None:
            out = hex_pooling(self.edges_level, self.device)
            residual = self.downsample(x, self.edges)
...
class chebnet_regression(nn.Module):
    ...
    def __init__(self, num_features,  block=GraphResidualBlock, layers=[2,2,2,2], conv_style=chebconv,activation_function=nn.LeakyReLU(), in_channels = 4, device='cuda'):
        ...
        self.fc = nn.Linear(num_features[3]*162, num_features[4])
